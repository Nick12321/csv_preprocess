...
data = pd.read_csv('condo_by_area_v4.csv')
...
# cat_cols = modified_data.select_dtypes(include=['object']).columns
# print(cat_cols)
cat_cols = set(cat_cols) - {label}
# print(cat_cols)

acc_sqft = []
acc_sqft = modified_data[cat_cols].iloc[0]
acc_sqft.append(modified_data[cat_cols].iloc[5])
print(acc_sqft)
print(modified_data[cat_cols].iloc[5])
# for x in mean_sqft:
#     mean_sqft = modified_data[cat_cols].iloc[x]
# mean_sqft = modified_data[cat_cols].iloc[0]
# print(mean_sqft)
